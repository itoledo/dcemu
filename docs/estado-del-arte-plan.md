...
| 4 | **El ARM7**: caché de predecodificación primero (hoy decodifica en cada paso), traductor ARM7→x64 sobre `jit_x64.c` si el escalón no alcanza; emisión por identidad de manejador del intérprete de `arm7.c`, que queda libre de SDL y enlazable por `tests/` | `notas-aica.md` / `arm7-plan.md` | **escalones 1 y 2a hechos** (2026-08-09/10): la predecodificación — entrada por palabra con validez por comparación (la regla de `jit_verificar`), manejadores por forma — vale **3,3-5,1 % de la corrida** (DOOM −3,6, CT −5,1, SR2 −3,3; rangos disjuntos), un cuarto a un tercio del ARM; y **los bloques en C** sobre ella (61,4 % de los pasos, 3,3 por bloque, cuatro teoremas de exactitud) valen **−0,4/−0,6/−1,8 %** más. Todas las compuertas al dígito. Queda **la emisión x64** sobre `arm7_blq_correr()`, el punto único donde reemplaza al lazo |
