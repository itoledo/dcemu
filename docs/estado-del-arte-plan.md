...
| 4 | **El ARM7**: caché de predecodificación primero (hoy decodifica en cada paso), traductor ARM7→x64 sobre `jit_x64.c` si el escalón no alcanza; emisión por identidad de manejador del intérprete de `arm7.c`, que queda libre de SDL y enlazable por `tests/` | `notas-aica.md` / `arm7-plan.md` | **escalón 1 hecho** (2026-08-09 noche): la predecodificación — entrada por palabra con validez por comparación (la regla de `jit_verificar`), manejadores por forma — vale **3,3-5,1 % de la corrida** (DOOM −3,6, CT −5,1, SR2 −3,3; rangos disjuntos), un cuarto a un tercio del ARM, con todas las compuertas al dígito. El reparto rehecho deja al ARM en 8,9-18,9 % → **el traductor sigue abierto** como escalón 2 |
