...
{
	short n = (arg >> 8) & 0x0F;
	BYTE m_bdisp = (arg & 0x00FF);
//	long source = disp*2 + PC + 4 - (mem_base);
	DWORD m_disp = m_bdisp*2 + PC + 4;
	WORD w;
...
{
	short n = (arg >> 8) & 0x0F;
	WORD m_wdisp = (arg & 0xFF);
	DWORD m_disp = m_wdisp*4 + (PC & 0xFFFFFFFC) + 4;

	ReadMemoryL(m_disp, (DWORD *) &R(n));
...
{
	short n = (arg >> 4) & 0x0F;
	BYTE m_bdisp = arg & 0x0F;
	BYTE valor = (BYTE) (R(0) & 0xFF);

	WriteMemoryB(R(n) + m_bdisp, &valor);
...
OPCODE(movw17) // MOV.W R0, @(disp, Rn) (10000001 nnnniiii)
{
	...
	WriteMemoryW(R(n) + m_bdisp*2, &valor);
...
OPCODE(movb19) // MOV.B @(disp, Rm), R0 (10000100 mmmmiiii)
{
	...
	memread(R(m) + m_bdisp, &b, sizeof(BYTE));
