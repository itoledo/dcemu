# La otra mitad de la deuda del medio texel: los juegos comerciales.
#
# **Es la pasada que apago el medio texel.** El parque de KOS lo dio por bueno
# --37 demos movidas, todas bien-- y aqui salio que el fondo de Street Fighter
# III se convierte en una rejilla de costuras. Ver CLAUDE.md, "Graphics
# pipeline", y herramientas/costuras.ps1, que es lo que lo conto.
#
# El parque de KOS no ejerce mipmaps, ni los modos de repeticion del TSP, ni
# los codigos de mezcla 2 y 3, ni el Offset Color (CLAUDE.md, "A KOS demo sweep
# cannot catch every regression"). Para esas rutas la linea base son los
# juegos, y la correccion del medio texel toca justamente el muestreo de
# textura.
#
# No hay linea base de imagen para los juegos, asi que lo que se comprueba es:
#
#   1. que cada uno sigue arrancando y DIBUJANDO (tiras > 0 en el resumen; una
#      captura negra y una lista vacia dan el mismo sintoma, y por eso se mira
#      la cuenta de tiras y no el BMP);
#   2. que la captura es DETERMINISTA (dos corridas del mismo brazo, mismo
#      hash) -- sin eso ninguna comparacion posterior vale;
#   3. que los dos brazos difieren solo donde deben: el que tiene texturas se
#      mueve, y el que no, no.
#
# Cada juego con --sin-vmu (una corrida escribe la tarjeta y la siguiente
# arrancaria de otra) y con las teclas del banco donde hacen falta.
...
function Correr($j, $brazo, $sufijo)
{
	...
	$bmp = "juego-$($j.n)-$brazo$sufijo.bmp"
...
foreach ($j in $juegos) {
	...
	"{0,-12} sin={1} con={2}  {3,-16} {4}" -f $j.n, $a1.h, $b1.h, $det, $mov
