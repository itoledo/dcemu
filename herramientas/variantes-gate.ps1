...
# el mismo binario -- con (por omision) y sin (DCEMU_JIT_SIN_VARIANTES_FPU=1: la busqueda ciega al modo
...
function Correr($b, $brazo)
{
	...
	Remove-Item env:DCEMU_JIT_SIN_VARIANTES_FPU -EA SilentlyContinue
	if ($brazo -eq 'sin') { $env:DCEMU_JIT_SIN_VARIANTES_FPU = '1' }

	$env:DCEMU_JIT = '2'; $env:DCEMU_RTC_FIJO = '1000000000'
	$env:DCEMU_CP_MS = "$($b.s * 1000)"

	$bmp = "logs\vf-$($b.n)-$brazo.bmp"
	$args = @("--salir-tras=$($b.s)", '--sin-vmu', "--captura-gl=$bmp")
	if ($b.wav) { $args += @('--sin-audio', "--captura-audio=logs\vf-$($b.n)-$brazo.wav") }

	& $Exe @args $b.img | Out-Null
	Copy-Item $err "logs\vf-$($b.n)-$brazo.txt" -Force
...
foreach ($b in $bancos) {
	...
	$base = Select-String -Path "logs\vf-$($b.n)-con.txt" -Pattern '^cp ' | ForEach-Object Line
	$c = Select-String -Path "logs\vf-$($b.n)-sin.txt" -Pattern '^cp ' | ForEach-Object Line
	...
	if ($b.wav) {
		$wc = (Get-FileHash "logs\vf-$($b.n)-con.wav" -Algorithm SHA256).Hash
		$ws = (Get-FileHash "logs\vf-$($b.n)-sin.wav" -Algorithm SHA256).Hash
		"{0,-7} wav identicos={1}" -f $b.n, ($wc -eq $ws)
	}
}

Remove-Item env:DCEMU_JIT,env:DCEMU_CP_MS,env:DCEMU_RTC_FIJO,env:DCEMU_JIT_SIN_VARIANTES_FPU,env:DCEMU_PULSAR_START,env:DCEMU_PULSAR_A,env:DCEMU_SOLO_A -EA SilentlyContinue
"hecho"
