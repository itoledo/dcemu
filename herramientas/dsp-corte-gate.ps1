# La compuerta del corte del programa DSP (aicadsp.c): DOS brazos en el mismo
# binario -- con (por omision) y sin (DCEMU_SIN_DSP_CORTE=1: los 128 pasos de
...
function Correr($b, $brazo)
{
	...
	$bmp = "logs\dspc-$($b.n)-$brazo.bmp"
	$args = @("--salir-tras=$($b.s)", '--sin-vmu', "--captura-gl=$bmp")
	if ($b.wav) { $args += @('--sin-audio', "--captura-audio=logs\dspc-$($b.n)-$brazo.wav") }

	& $Exe @args $b.img | Out-Null
	Copy-Item $err "logs\dspc-$($b.n)-$brazo.txt" -Force
...
foreach ($b in $bancos) {
	...
	$base = Select-String -Path "logs\dspc-$($b.n)-con.txt" -Pattern '^cp ' | ForEach-Object Line
	$c = Select-String -Path "logs\dspc-$($b.n)-sin.txt" -Pattern '^cp ' | ForEach-Object Line
	...
	if ($b.wav) {
		$wc = (Get-FileHash "logs\dspc-$($b.n)-con.wav" -Algorithm SHA256).Hash
		$ws = (Get-FileHash "logs\dspc-$($b.n)-sin.wav" -Algorithm SHA256).Hash
		"{0,-7} wav identicos={1}" -f $b.n, ($wc -eq $ws)
	}
}

Remove-Item env:DCEMU_JIT,env:DCEMU_CP_MS,env:DCEMU_RTC_FIJO,env:DCEMU_SIN_DSP_CORTE,env:DCEMU_PULSAR_START,env:DCEMU_PULSAR_A,env:DCEMU_SOLO_A -EA SilentlyContinue
"hecho"
