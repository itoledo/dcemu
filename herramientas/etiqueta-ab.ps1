# etiqueta-ab.ps1 -- la tanda A/B de la etiqueta viva de la cache de
# traducciones (mmu.h): un binario, palancas de ambiente, la etiqueta leida de
# mmu_etiqueta contra construida en cada acceso
# (DCEMU_MMU_ETIQUETA_CALCULADA=1, la emision anterior byte por byte).
...
# no traduce nada y no mira la etiqueta. Crazy Taxi va igual, de testigo.
param(
...
Write-Output "hash: $((Get-FileHash $Exe -Algorithm SHA256).Hash.Substring(0,16))  etiqueta viva contra calculada, guest $Guest"
...
function Correr($calculada) {
	foreach ($v in @("DCEMU_PULSAR_START","DCEMU_PULSAR_A","DCEMU_SOLO_A","DCEMU_MANDO","DCEMU_CP_MS","DCEMU_JIT","DCEMU_MMU_ETIQUETA_CALCULADA")) {
	...
	if ($calculada) { $env:DCEMU_MMU_ETIQUETA_CALCULADA = "1" }
...
$brazos = @(@{ n = "viva"; calc = $false }, @{ n = "calculada"; calc = $true })
...
$a = $res["viva"] | Measure-Object -Minimum -Maximum -Average
$c = $res["calculada"] | Measure-Object -Minimum -Maximum -Average
$pares = 0
for ($r = 0; $r -lt $Rondas; $r++) { if ($res["viva"][$r] -lt $res["calculada"][$r]) { $pares++ } }
$disjuntos = ($a.Maximum -lt $c.Minimum) -or ($c.Maximum -lt $a.Minimum)
$pct = ($a.Average - $c.Average) / $c.Average * 100
"{0}: viva {1}-{2} ms, calculada {3}-{4} ms, {5:+0.0;-0.0} % en la media, {6}/{7} pares a favor de la viva, rangos {8}" -f `
	$Guest, $a.Minimum, $a.Maximum, $c.Minimum, $c.Maximum, $pct, $pares, $Rondas,
	$(if ($disjuntos) { "DISJUNTOS" } else { "solapados" })

foreach ($v in @("DCEMU_RTC_FIJO","DCEMU_PULSAR_START","DCEMU_PULSAR_A","DCEMU_SOLO_A","DCEMU_MMU_ETIQUETA_CALCULADA")) {
	Remove-Item "Env:$v" -ErrorAction SilentlyContinue
}
