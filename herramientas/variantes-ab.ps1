# La tanda de las variantes FPU del traductor (jit.c): DOS brazos sobre
# el mismo binario -- con (por omision) y sin (DCEMU_JIT_SIN_VARIANTES_FPU=1, la busqueda ciega al modo
# de siempre) -- bajo el traductor, que es la forma que la adopcion entrega.
# Calentamiento POR GUEST descartado, orden alternado entre rondas.
#
# ciclo de PGO primero (GEN -> pgo.ps1 -Clang -> USE): aicadsp.c cambio, el
...
Remove-Item env:DCEMU_JIT,env:DCEMU_JIT_SIN_VARIANTES_FPU,env:DCEMU_PULSAR_START,env:DCEMU_PULSAR_A,env:DCEMU_SOLO_A -EA SilentlyContinue
Write-Output "=== fin"
