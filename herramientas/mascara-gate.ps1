# La compuerta del mascara de canales activos (aica.c): DOS brazos en el mismo
# binario -- con (por omision) y sin (DCEMU_SIN_MASCARA_CANALES=1: los 128 pasos de
# siempre). La baranda que manda es el .wav de Crazy Taxi (78 pasos de
# reverberacion programados: exactamente el caso que el corte recorta), mas
...
function Correr($b, $brazo)
{
	...
	Remove-Item env:DCEMU_SIN_MASCARA_CANALES -EA SilentlyContinue
	if ($brazo -eq 'sin') { $env:DCEMU_SIN_MASCARA_CANALES = '1' }

	$env:DCEMU_JIT = '2'; $env:DCEMU_RTC_FIJO = '1000000000'
	$env:DCEMU_CP_MS = "$($b.s * 1000)"

	$bmp = "logs\masc-$($b.n)-$brazo.bmp"
	$args = @("--salir-tras=$($b.s)", '--sin-vmu', "--captura-gl=$bmp")
	if ($b.wav) { $args += @('--sin-audio', "--captura-audio=logs\masc-$($b.n)-$brazo.wav") }

	& $Exe @args $b.img | Out-Null
	Copy-Item $err "logs\masc-$($b.n)-$brazo.txt" -Force
...
foreach ($b in $bancos) {
	...
	$base = Select-String -Path "logs\masc-$($b.n)-con.txt" -Pattern '^cp ' | ForEach-Object Line
	$c = Select-String -Path "logs\masc-$($b.n)-sin.txt" -Pattern '^cp ' | ForEach-Object Line
	...
	if ($b.wav) {
		$wc = (Get-FileHash "logs\masc-$($b.n)-con.wav" -Algorithm SHA256).Hash
		$ws = (Get-FileHash "logs\masc-$($b.n)-sin.wav" -Algorithm SHA256).Hash
		"{0,-7} wav identicos={1}" -f $b.n, ($wc -eq $ws)
	}
}

Remove-Item env:DCEMU_JIT,env:DCEMU_CP_MS,env:DCEMU_RTC_FIJO,env:DCEMU_SIN_MASCARA_CANALES,env:DCEMU_PULSAR_START,env:DCEMU_PULSAR_A,env:DCEMU_SOLO_A -EA SilentlyContinue
"hecho"
