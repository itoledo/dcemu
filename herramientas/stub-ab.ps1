...
#
# La tanda del talon de sincronizacion por bloque: dos brazos sobre el mismo binario, con
# (por omision) y sin (DCEMU_JIT_SYNC_EN_CADA_TALON=1, por manejador). El cliente es
...
{
	foreach ($v in @("DCEMU_PULSAR_START","DCEMU_PULSAR_A","DCEMU_SOLO_A","DCEMU_JIT_SYNC_EN_CADA_TALON")) {
	...
	$env:DCEMU_JIT = "2"
	if ($brazo -eq "sin") { $env:DCEMU_JIT_SYNC_EN_CADA_TALON = "1" }
...
foreach ($v in @("DCEMU_JIT","DCEMU_JIT_SYNC_EN_CADA_TALON","DCEMU_PULSAR_START","DCEMU_PULSAR_A","DCEMU_SOLO_A")) {
	Remove-Item "Env:$v" -ErrorAction SilentlyContinue
}
