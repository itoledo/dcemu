...
(el árbol extraído del instalador oficial — `E:\llvm\22.1.8` en la máquina del banco original,
`C:\llvm\22.1.8` en la segunda; `herramientas\banco.ps1` lo resuelve y `llvm-entorno.ps1` arma el
entorno) + lld-link + ThinLTO sobre el mismo fuente, con `-ffp-contract=off` (el `/fp:precise` de clang sí contrae a FMA y la
...
**Dos máquinas corren este árbol y `herramientas\banco.ps1` es donde viven las rutas que cambian
entre ellas** (2026-09-03): LLVM (`E:\llvm\22.1.8` / `C:\llvm\22.1.8`, o `DCEMU_LLVM`), el trío CHD
(`E:\Juegos\roms\dreamcast` / `roms\chd`, o `DCEMU_CHD`) y las imágenes del banco por nombre corto
(`ImagenBanco doom|ct|sr2|vt`) — Sega Rally 2 es el `.gdi` v1.003 en una máquina y el `.chd` de
redump en la otra, mismo guest para la compuerta y no para el cronómetro entre máquinas (el `.chd`
paga la descompresión; los absolutos ya sólo se comparan dentro de un binario y una máquina). Los
guiones del banco vigente (`pgo.ps1`, `llvm-entorno.ps1`, `ociosos-*.ps1`, `chd-*.ps1`) resuelven por
ahí; los históricos conservan sus rutas escritas y corren en la máquina donde se escribieron. Dos
reglas pagadas: `Test-Path` a secas expande comodines y **`[!]` casa con `!`**, así que el `.gdi` de
SR2 y el `.cdi` de Virtua Tennis no existen para él aunque estén (`-LiteralPath` siempre); y el
generador NMake/Ninja deja el ejecutable en la raíz del directorio de compilación, no en `Release\`
(`ExeBanco`). Las imágenes adicionales están en el NAS: `\\birdienas\backup\dcemu\roms` (DCDoom, el
repack de Virtua Tennis, Crazy Taxi y Capcom vs. SNK de DCRES, mame4all) y
`\\birdienas\downloads\transmission\completed\sega-dreamcast-redump-collection` (57 `.chd` redump,
entre ellos SR2, el trío CHD, MKG, CT2, VT y el resto del barrido de `chd-barrido.ps1`).
...
flash is synthesized. **`ip.bin` is needed too, but only to run a loose `.bin`** — the whole KOS
park is loose `.bin`s, so without it every demo dies at startup with `No se pudo abrir ip.bin`,
which is what silently emptied a full sweep (see the sweep bullet in "Measurement discipline").
It is not in the repo either; any Katana IP.BIN does. An argument ending in `.bin` loads `ip.bin` plus that binary directly;
...
| `--hilos` / `--sin-hilos` | el AICA y el ARM7 en su propio hilo (`hilo_aica.c`). **Por omisión desde el 2026-09-05**: exacto hasta la lista de entregas en los tres guests (`linea-gate.ps1`, cinco brazos sobre el canónico entregado) y parque KOS 151/151 sin una diferencia; tanda sobre ese canónico (`F79CFE145D3ED123`, reentrenado con hilos), en reposo y sin usuario: **DCDoom −10,4 %, Sega Rally 2 −7,7 %, Crazy Taxi −11,3 %**, los tres con rangos disjuntos y 4/4 — marcas **CT 3,06×, DOOM 1,67×, SR2 1,37×**. `--sin-hilos` es la palanca de aislamiento y el brazo de control de todo A/B — la omisión cobra la trampa de la adopción del traductor: un guion que «no pasaba `--hilos`» ahora corre con hilos. Ver `DCEMU_HILO_AICA_GIRO` y `docs/hilos-plan.md` |
...
| `DCEMU_JIT_VOLCADO=ARCHIVO` | vuelca el código emitido en crudo y lista dónde quedó cada bloque, para desensamblarlo. Es el desarme del riesgo «el emisor mismo» del lado del binario; del lado del código lo es `tests/test_jit_x64.c`. **En estas máquinas no hay `objdump`**: el desensamblador es el de LLVM 22.1.8 (`E:\llvm\22.1.8` o `C:\llvm\22.1.8` según la máquina — fuera de `Program Files` y fuera del árbol de VS a propósito, porque el toolset de Visual Studio ya no lo trae; el entorno se arma punteando `herramientas\llvm-entorno.ps1`, que lo encuentra por `banco.ps1`, y el porqué está en `docs/jit-sota-plan.md`, fase G paso 0), y como `llvm-objdump` no lee binarios crudos, el crudo se envuelve primero — `llvm-objcopy -I binary -O elf64-x86-64 v.bin v.o` y después `llvm-objdump -D --triple=x86_64 --section=.data v.o`. Ese LLVM **tampoco trae `llvm-mc`**, pero el ensamblador integrado de clang cubre lo mismo: `clang -c a.s -o a.o` sobre un `.s` con `.intel_syntax noprefix` |
...
| `DCEMU_SONDA_ENTREGAS=1` | una línea por interrupción entregada (`reloj_total`, INTEVT, PC): **el árbitro de exactitud para cambios de temporización de entregas** — los `cp` por ms no sirven ahí porque muestrean en las entradas del bloque periódico y un cambio de cadencia mueve el instante del muestreo con el guest intacto. Es la sonda que tumbó el rearme condicional de `intc_sh4_reintentar` (2026-08-31): armar solo con alguien pidiendo ahorraba el 61 % de los 24,6 M de servicios solo-reintento de DOOM y la lista de entregas divergía (132 313 contra 134 655, en cualquier dirección) — porque **la grilla del grano no es absoluta**: cada servicio consume el acumulado y corre la fase de los granos siguientes, así que quitar servicios mueve las fronteras donde una entrega espera. Con esa grilla, la única poda exacta del bloque periódico es la del cuerpo (el servicio partido de B.3), ya medida neutra: candidato cerrado por imposibilidad. El expediente en `docs/notas-tiempo.md`; el censo del reintento quedó en `--perf` |
...
| `DCEMU_JIT_TABLA_VIEJA=1` | vuelve la tabla hash de bloques a su conducta anterior (2026-08-30): sondeo de 8, sin reuso de lápidas y sin desmarcar al fallar — **con su fuga**. El censo de los «sin lugar» de SR2 (12 904) dio 9 PCs, no 12 904 fríos: una inserción sin lugar no volvía al intérprete sino que se **retraducía en cada visita** (el dedup usa el mismo buscar), y los cúmulos que la hacían fallar eran las **lápidas de sus propias retraducciones**, que nunca salen de la tabla — sitios remapeados de WinCE, 3 PCs con el 82 %. La fuga quemaba una ranura de `jit_bloques[]` por visita hasta **chocar el tope de 32 768 a mitad de la corrida de 60 s**, y chocado no se traduce NUNCA MÁS (retraducciones por remapeo incluidas) — sin ningún contador que lo dijera. Arreglo: sondeo a 32 (falla ~0,25³², extinta), reuso de lápidas al insertar (no corta cadenas: buscar las trata como ocupadas), desmarcar si aun así falla, y el tope ahora avisa y se cuenta. SR2 queda en 0 sin lugar y 29 073/32 768 con el traductor vivo; DOOM/CT tenían 0. La emisión no cambia: A/B sobre un binario. Compuerta verde (capturas y `DCEMU_CP_MS` completos byte a byte en SR2/DOOM; CT byte a byte **bajo replay** — sin replay divergía un ciclo por el pad, correlacionando con el brazo por accidente: el descarte del jitter es el replay, no la reproducibilidad), tanda neutra (SR2 solapado, DOOM al dígito): es el cierre de un modo de falla que el banco de 60 s apenas roza, no una optimización. Residuo nombrado: 12 019 rechazos con el tope de retraducción en SR2 — sitios que alternan >16 veces, el caso «variantes por ASID» |
| `DCEMU_JIT_BLOQUES=N` | recorta el tope de bloques del traductor en runtime (omisión y máximo 65 536; `32768` reproduce el tope anterior y es el brazo del A/B). El sondeo de 180 s convirtió el tope viejo en defecto medido: **SR2 lo llenaba legítimamente y el traductor moría a mitad de sesión** — 9577 propuestas rechazadas con la tabla llena, y una lápida de retraducción que no puede renacer pierde el bloque entero. Con 65 536 (tabla hash de ints a 18 bits, arena a 256 MB) SR2-180 queda en 39 406 ranuras con el traductor vivo. Compuerta verde (capturas + cp completos en SR2 180 s y DOOM); tanda sobre el canónico reentrenado `25B7366C16CED290`: **SR2 −1,4 % a 180 s con rangos disjuntos y 4/4** — la ganancia vive en el tramo donde el tope viejo mataba al traductor, por eso el banco de 60 s no la veía. Ver `docs/recompilador-plan.md` |
| `DCEMU_MMU_ATAJO_TARDE=1` | el atajo de P1/P2/P4 vuelve a ir **detrás** de la consulta a la caché de datos de la MMU, que es la conducta anterior. Por omisión va delante, como en el código emitido desde la fase 6: P1, P2 y P4 no se traducen y **nunca se guardan**, así que su ranura queda sin estrenar para siempre y cada acceso pagaba índice, etiqueta y cuatro comparaciones para fallar siempre. Son **69,6 M de accesos en 20 s emulados de DCDoom (6,0 % de sus traducciones)**. Exacto por construcción y verificado (capturas y puntos de control idénticos en DCDoom y Sega Rally 2 en los dos brazos). **Medido NEUTRO en el reloj**: DOOM −1,0 % y SR2 −0,35 %, los dos con 3 de 4 rondas a favor y rangos solapados, o sea por debajo del estándar del árbol — y la aritmética lo anticipa, 70 M de sondeos a una tabla en L1 son milisegundos. Queda encendido porque es estrictamente menos trabajo y con palanca para el A/B |
| `DCEMU_MMU_ETIQUETA_MODO=1` | la etiqueta de la caché de entrada de la MMU vuelve a llevar el **modo siempre** — la conducta anterior. El censo en tres escalones (contadores `perf_mmu_ent_*`) mostró que el 98,3 % (DOOM) / 91,4 % (SR2) de los fallos por etiqueta eran **solo el modo con el mismo ASID** — el vaivén usuario/privilegiado de WinCE — y con ASID igual el recorrido da la misma entrada en ambos modos. La omisión nueva: dos formas de etiqueta decididas al llenar (`MMU_CACHE_AMBOS` sin modo si el ASID casó por igualdad o la página es SH; con modo si casó vía espacio único — la trampa por la que el bit existía). Aciertos **43,4→84,1 % en DOOM** (recorrido 20,9→8,3 de 64) y 57,9→72,3 % en SR2 — ojo: **esas tasas están medidas con el denominador roto** que el 2026-09-04 se corrigió (el atajo P1/P2 emitido contaba una traducción sin acierto, y en DCDoom son 607,8 M de 1163 M accesos); el A/B de esa fase fue sobre el reloj y sigue en pie, pero la tasa absoluta hoy se lee 98,6 % y no se compara con estas —; compuerta verde (capturas + cp completos); tanda: **DOOM −3,0 % con rangos disjuntos y 4/4**, SR2 en su dispersión. El residuo es la generación vencida (los LDTLB de WinCE), que ninguna etiqueta arregla. Ver `docs/mmu-plan.md`, «La etiqueta sin modo» |
| `DCEMU_MMU_ETIQUETA_CALCULADA=1` | la etiqueta de la caché de traducciones vuelve a **construirse en cada acceso** — la emisión anterior byte por byte. Por omisión vive en `mmu_etiqueta` y el código emitido la lee con **una carga en vez de diez instrucciones**, dos de ellas cargas dependientes (el puntero a PTEH y con él PTEH) delante de la comparación que decide el camino rápido. La mantienen los cuatro sitios que pueden moverla (las dos entradas de `UpdateSR()`, la escritura del guest a PTEH y el reset de la CPU) y el bloque periódico verifica la coherencia una vez por servicio, con el contador en el resumen — **que encontró un agujero el día que se escribió**: `mmu_reset()` calculaba con SR en cero y el SR de reset se escribe a mano, así que el arranque entero corría con la etiqueta de modo usuario. Tanda sobre el canónico `57BC6AB6B6168F89`: **DCDoom −2,1 % con rangos disjuntos y 4/4**, Sega Rally 2 −1,0 % con 3/4 y solape, Crazy Taxi inerte por construcción (en modo plano no se emite traducción). La emisión baja **64 bytes**: la traducción vive en dos rutinas compartidas, así que lo que se gana es latencia de camino crítico, no icache. Ver `docs/mmu-plan.md` |
| `DCEMU_MMU_URC_INMEDIATO=1` | el contador de reemplazo de la UTLB (`MMUCR.URC`) vuelve a avanzarse **en cada acceso emitido**, la emisión anterior byte por byte. Un acierto de caché sigue siendo un acceso a la UTLB y tiene que avanzarlo (es lo que decide qué entrada reemplaza el `LDTLB` del guest), pero **nadie lo mira en el camino caliente**: se observa en tres sitios — el `LDTLB`, una lectura del guest a MMUCR y los puntos de control — y entre observaciones sólo importa cuántos avances hubo. Por omisión el acceso suma uno a `mmu_urc_pend` (**una instrucción** en vez de veinte con lectura-modificación-escritura de MMUCR y dos ramas de envolvimiento) y esos tres sitios materializan; una escritura del guest a MMUCR **descarta** lo pendiente en vez de aplicarlo. La forma cerrada está **probada antes que el emisor** como la de DIV1 (URC se envuelve en 64 y, si URB no es cero, también en URB, así que no es sumar módulo 64): `tests/test_mmu.c` compara `mmu_urc_tras()` contra los pasos de a uno sobre el espacio entero, 64×64 pares por 130 valores de N. Tanda sobre el canónico `0577D82BE7D5E77B`: **Sega Rally 2 −3,4 % y DCDoom −2,7 %, los dos con rangos disjuntos y 4/4** — la mayor ganancia de la MMU desde el atajo P1/P2 y la primera que gana en los dos guests con MMU —, Crazy Taxi inerte por construcción (modo plano: cero pendientes al salir). La emisión baja **112 bytes**: como con la etiqueta, lo que se gana es latencia por acceso y no icache. Los puntos de control imprimen MMUCR, así que la compuerta compara el URC materializado. Ver `docs/mmu-plan.md` |
| `DCEMU_MMU_EMISION_VIEJA=1` | el índice de la caché de traducciones vuelve a calcularse con `imul` y la máscara vuelve a negarse al vuelo, la emisión anterior byte por byte. La entrada de `mmu_datos[]` pasó de 28 a **32 bytes** y el campo que la agranda **no es relleno**: es la máscara ya negada. Tres cosas salen de eso, todas en la cabeza de la cadena del acceso — el índice deja de multiplicar (`shl 5` en vez de un `imul` de tres ciclos **delante** de la primera carga), la entrada deja de cruzar líneas de caché (con 28 bytes, 7 de cada 16 quedan a caballo, y el camino rápido lee la entrada completa) y el `not` se va. La palanca aísla la mitad emitida dentro de un binario; el tamaño del struct es compilación y obliga a dos. **Tanda de la mitad emitida** (canónico `93C117A032E8EA28`): **DCDoom −1,0 % con rangos disjuntos y 4/4**, Sega Rally 2 −0,1 % solapado, Crazy Taxi inerte por construcción (−0,4 %, que es el ruido de esa tanda). **Y la tanda de dos binarios no decide, con la prueba adentro**: Crazy Taxi, que no emite traducción y cuyo código no se mueve, salió **−0,7 % con rangos disjuntos y 4/4** — contaminación de disposición más grande que el efecto buscado, así que el +0,4 % de DCDoom y el −0,1 % de SR2 no dicen nada. La lección que queda: **en una tanda de dos binarios hay que correr un guest que el cambio no pueda tocar, porque es el medidor de ruido**, y repetir no lo arregla (el sesgo es sistemático por binario, no aleatorio). Se queda porque la mitad aislable gana donde la medición distingue, la otra es su habilitador y el conjunto es estrictamente menos trabajo. La emisión no cambia de tamaño: los seis bytes viven en las dos rutinas compartidas y los absorbe la alineación. Ver `docs/mmu-plan.md` |
| `DCEMU_MMU_PERMISO_APARTE=1` | la etiqueta y el permiso de la caché de traducciones vuelven a comprobarse en **dos comparaciones**, la emisión anterior byte por byte. Por omisión la entrada guarda **la etiqueta por tipo de acceso** — la etiqueta cuando el permiso ya pasó, cero cuando no — y una sola comparación contesta las dos preguntas: el cero no puede confundirse con una etiqueta viva porque todas llevan `MMU_CACHE_VALIDA`, y los dos desenlaces que se funden ya eran el mismo (el ayudante). No cambia el tamaño de la entrada **ni los desplazamientos**, así que la palanca reproduce la emisión vieja exacta y la pregunta cabe entera en un binario. Precio dicho de frente: los dos motivos de rechazo del censo se funden en uno, y el resumen lo dice. **Medido dos veces** sobre el canónico `1555E6699C14A284`, con las seis rondas decididas antes de mirar las cuatro: **DCDoom −0,7 % las dos veces, 10 de 10 pares**; Sega Rally 2 −1,3 % y −1,7 %, 9 de 10; **rangos solapados en las seis filas**, o sea por debajo del estándar estricto. Lo que lo hace firme es el testigo: Crazy Taxi es inerte por construcción y sale **5 de 10 pares y ±0,0 %**, que es lo que da un efecto inexistente — 10/10 contra un control de 5/10 no es 4/4 sin control. La lección: **cuando la dispersión tapa el efecto hay que cambiar de estadístico, no de conclusión**, y los pares acumulados sobre dos tandas con un guest inerte de calibración dicen más que cuatro rondas disjuntas. La emisión baja **32 bytes exactos** (el `test` y su rama, 16 en cada una de las dos rutinas compartidas), que es justo lo que predice la aritmética. Ver `docs/mmu-plan.md` |
...
| `DCEMU_JIT_TRAD_EN_LINEA=1` | vuelve a emitir la **traducción MMU en línea en cada sitio de acceso** (2026-08-25), la conducta anterior. Por omisión el cuerpo de `gen_traducir_mmu` se emite UNA vez como dos **rutinas compartidas** (lectura/escritura) al frente del arena y cada sitio llama por rel32, conservando el atajo P1/P2 en línea y decidiendo su camino lento con el EAX devuelto. La cadena de sondas que lo encontró: reparto (SR2 a 7,4 ns/instr contra 3,6 de CT) → censo de accesos (254 B emitidos/instr contra 94) → censo de bytes por plantilla (`DCEMU_JIT_SONDA_BYTES=1`, permanente): **un `MOV.L @Rm,Rn` emitía 320 bytes contra 62 del modo plano** y los accesos eran ~60 % del arena — presión de icache. Con las rutinas: arena de SR2 **169,6 → 97,4 MB**, y la tanda (`02EE89A0671B8A51`) dio **SR2 −11,1 % y DOOM −7,2 %, ambos con rangos disjuntos y 4/4** — la mayor ganancia desde el índice de enlaces; CT inerte por construcción (plano: cero llamadas). Marcas: SR2 ~1,10×, DOOM ~1,35×. La sonda de accesos fuerza la forma en línea (razones por sitio). De paso quedaron destapados y nombrados dos topes silenciosos: **el arena de 192 MB** (SR2 lo chocaba con la emisión en línea y dejaba de traducir sin contador — ahora `sin arena` en el resumen) y la **tabla de 32 768 bloques** — cuyo censo (2026-08-30) resultó ser una fuga, ver `DCEMU_JIT_TABLA_VIEJA` |
| `DCEMU_JIT_SYNC_PREVIA=1` | la **sincronización previa a un acceso vuelve al camino rápido**, delante de la plantilla — la conducta anterior. Por omisión se emite **en los talones lentos**, justo antes de la llamada al ayudante, que es el único sitio desde donde una instrucción emitida puede faltar: el camino rápido no puede (alineación, modo resuelto al emitir, traducción acertada, zona con base directa — con el watchpoint y el UBC plegados en `mem_directo_recalcular()` — y la página con código desvía). El censo del contrato (2026-09-02) lo puso primero: los accesos son la clase mayor, **36,55 % de DOOM, 39,38 % de SR2, 41,24 % de CT**, y cada uno pagaba diez instrucciones de volcado que el 99,99 % de las veces no servían. Lo que la hace exacta sin listas que mantener es que **toda salida a C pasa por `gen_llamar()` o `tr_manejador()`**, y las dos la emiten antes de llamar; el conductor la ARMA y comprueba que alguien la usó, con `jit_sync_sin_consumir` en el resumen (cero siempre). Dos reglas pagadas: **el contador de instrucciones vivía adentro de la sincronización** (moverla perdió mil millones de instrucciones en 20 s de DOOM — el arreglo es contar el intento con un `inc` de un byte ANTES de la plantilla, como `run()`), y **bajo MMU la emisión crece** porque un acceso abre dos talones y los dos la llevan (DOOM 39,4 → 44,9 MB); en modo plano hay un solo talón y es neutro en bytes
| `DCEMU_JIT_SYNC_EN_CADA_TALON=1` | el volcado de la sincronización vuelve a emitirse **entero en cada talón**, la forma anterior byte por byte. Por omisión se emite **una vez por bloque** y cada talón lo llama por `rel32`: el volcado son las mismas seis tiendas para todos los accesos de un bloque (las ranuras que ese bloque mapea, más CYC), así que lo único propio de la instrucción es el `mov` del PC. El `call` es seguro dentro de un bloque aunque el arena tenga una sola información de desenrollado — el talón no llama a nadie ni toca la pila más allá del retorno, y nada puede faltar mientras está corrido. **Vale −15,4 % del arena de DCDoom** (49,2 → 41,7 MB), que devuelve dos tercios del crecimiento que `DCEMU_JIT_SYNC_PREVIA` había traído bajo MMU |
| `DCEMU_JIT_SIN_DIR_CONSTANTE=1` | los accesos con **dirección constante** vuelven al camino genérico, la conducta anterior byte por byte. Son los literales de PC-relativo (`MOV.L @(d,PC),Rn` y `MOV.W @(d,PC),Rn`), cuya dirección se decide al traducir — `(pc & ~3) + 4 + disp*4` — y que el censo del contrato puso en el **13,87 % de las instrucciones ejecutadas de Crazy Taxi**, 6,21 % de DCDoom y 6,82 % de Sega Rally 2. La dirección ya se cargaba como inmediato; lo que se pliega es **el camino hasta el dato**: la guarda de alineación no se emite (la fórmula ya alinea), el índice de zona entra en el desplazamiento de la carga de la tabla y el offset dentro de la zona es un inmediato. Lo único que sigue mirando el estado es la prueba de base nula, y **tiene que seguir**: la tabla cambia al armar un watchpoint o un break de operando. El valor se sigue leyendo — el literal es dato. **CT −2,22 % con rangos disjuntos y 4/4** (canónico `3E7AF60F89EE8F1A`), marca 2,80×; DCDoom neutro **por construcción**, porque el pliegue de zona vale sólo en modo plano — bajo MMU la física sale de la rutina de traducción y no es constante, así que a esos guests sólo les queda la elisión de la alineación |
| `DCEMU_JIT_CORTE_VIEJO=1` | el **corte del bloque periódico vuelve a emitirse como dos comparaciones** (`cmp CYC,400` contra la constante y `cmp` contra la bandera de reintento), la conducta anterior byte por byte. Por omisión es **una sola**: `intc_corte_limite` (intc.h) vale `RELOJ_GRANO` y **cero mientras el reintento esté armado**, así que `CYC >= límite` es cierta exactamente cuando lo era la disyunción de `main_loop`. Dos instrucciones en vez de cuatro en casi todas las fronteras del bloque — el censo del contrato dice que el 90 % de las instrucciones lleva corte. Las dos cosas se mueven **juntas**, con `INTC_PEDIR_REINTENTO()`/`INTC_LIMPIAR_REINTENTO()`, y el bloque periódico verifica la coherencia una vez por servicio: un sitio que armara el reintento sin envenenar el límite dejaría al traductor sin cortar donde el intérprete corta, que es una divergencia silenciosa y tardía. El contador `intc_corte_incoherente` va en el resumen, sin condición y en cero
| `DCEMU_JIT_DIV1_EMITIDA=1` | **DIV1 emitida en línea y sin ramas: medida y APAGADA.** Por omisión va por el manejador real, que es lo que ganó. El censo del contrato la señalaba como el blanco mayor de DCDoom — **4,91 % de sus instrucciones ejecutadas**, división por software — y cada paso paga sincronización, llamada, recarga de CYC y recarga de las cinco ranuras, más un manejador con dos switch anidados. La forma cerrada que permite emitirla recta: se **resta si Q == M** y se suma si no; `Q' = qs ^ tmp1 ^ M` con `qs` el bit 31 de Rn antes del corrimiento y `tmp1` el préstamo/acarreo; `T' = (Q' == M)`. Está **probada antes que el emisor** (`tests/test_arith.c`, `div1_la_forma_cerrada_coincide`: 4096 estados al azar más los bordes contra el manejador real), y sin `cmov` — que el emisor no tiene — el operando se niega con máscara (`b' = (b ^ (sel-1)) - (sel-1)`, así **siempre se suma**), con `tmp1 = CF ^ !sel` y un `and` contra `(b != 0)` para el único caso que eso no cubre, **b == 0**, donde acarreo y préstamo dejan de ser complementarios. **Perdió su A/B: +3,6 % en DOOM, rangos disjuntos y 4/4** (canónico `E1E565F833F4738B`, totales al dígito). El arena descarta lo fácil — 49 474 303 bytes contra 49 239 871, **+0,48 %** — así que no son los bytes: **las ramas del manejador se predicen bien** (M es fijo durante una división y el patrón de Q lo aprende el predictor) y la versión sin ramas las cambia por una cadena de dependencias de quince pasos con dos `setcc`/`movzx` y un lee-modifica-escribe de SR. Es la lección del cuerpo rápido del DSP, ahora en el SH-4: **quitar ramas que ya se predicen no compra nada, y la aritmética que las reemplaza se paga en latencia**. Queda con palanca porque la forma cerrada está probada y porque la variante que *no* se midió — emitir el switch tal cual, con sus ramas, para ahorrar sólo la llamada y las cinco recargas — tiene aquí la mitad del trabajo hecha. La palanca cambia **la fila entera** (`{ciclos 1, accede 0}` emitida contra `{0, 1}` por manejador), no sólo el emisor: el manejador suma sus ciclos por dentro y el conductor no debe volver a sumarlos |
| `DCEMU_JIT_OCIOSOS=0\|1\|2` | la **elisión de lazos ociosos** (2026-09-03), en tres posiciones porque son dos mecanismos: `0` la apaga entera (la emisión anterior byte por byte), `1` emite sólo los **bumps** de la generación de impureza (mide su costo) y `2` —la omisión— instala además las **sondas**. El principio: dentro de un grano no corre nada externo al guest, y en modo plano el camino rápido emitido sólo entra en RAM del sistema, así que una vuelta de un lazo que devuelve los mismos R0-R15/SR/PR/GBR/MACH/MACL sin escritura emitida, sin llamada a C y sin entrada al despachador es idéntica a todas las que siguen hasta el corte: se saltean k vueltas sumando k por ciclos y k por instrucciones (`k = (límite − 1 − CYC) / ciclos_por_vuelta`), la última parcial corre de verdad y el corte cae en la misma instrucción que en el intérprete — la grilla no se mueve y **el total de instrucciones queda al dígito**. La sonda vive en un talón por arista de retroceso plana y estática (el parche del enlace lo instala), compara en C contra la instantánea de la vuelta anterior y se **retira** a los 16 fallos seguidos reparcheando el enlace directo. Bajo MMU no entra ni emite bumps (v1): DCDoom y SR2 inertes por construcción. **Crazy Taxi elide el 36,4 % de sus instrucciones** (una sola arista, `0c158400 → 0c1583f8`, 20 instrucciones por vuelta, 8,8 vueltas por elisión), compuerta verde contra el intérprete en los tres brazos con 60 000 puntos y el total al dígito. Tanda de tres brazos sobre un binario (CT 180 s, 4 rondas rotadas): **entera 73 766–74 781 ms contra apagada 76 880–78 009, −4,7 % con rangos disjuntos y 4/4; solo-bumps 76 974–78 539, neutro** — el costo entero es de las sondas y los 7,2 M de bumps por segundo no se ven en el reloj. Marca 2,42× sobre el binario MSVC sin PGO (venía de 2,34×). **Sobre el canónico clang reentrenado de la misma máquina (`35AA2D4CFA5B62B5`, 2026-09-03), compuerta verde en los tres guests** — capturas byte a byte contra el intérprete y contra el binario MSVC, 155 000 puntos y los totales al dígito — **y tanda de tres brazos y tres guests: CT entera 66 162–67 146 ms contra apagada 69 032–71 965, −4,8 % con rangos disjuntos y 4/4, bumps 69 529–71 353 neutro; DCDoom (22 968–23 259 / 23 108–23 428 / 23 108–23 335) y Sega Rally 2 (46 774–47 887 / 46 761–47 685 / 46 677–47 680) solapados** — inertes por construcción y ahora medidos: 35 y 3 aristas con sonda, 360 y 6 sondas, 0 elisiones, bumps sólo en el arranque plano. Marca de CT 2,69× sobre ese canónico. **El censo del 2026-09-04 la cierra**: la generación de impureza va partida en cuatro clases (escritura, acceso, manejador, despacho) y los enlaces sin sonda se cuentan por motivo, y con eso Crazy Taxi resulta estar **en su tope estructural** — de 31,1 M de sondas, 15,5 M eliden y casi todo el resto falla por `despacho` (15 542 362 contra 30 134 / 27 506 / 28 765 de las otras tres), o sea la vuelta que toma la instantánea tras entrar al despachador; cero sin lugar antes del corte. Virtua Tennis y Capcom vs. SNK **no tienen lazos ociosos**: 10 065 y 7368 sondas en corridas de 7,8 y 5,7 mil millones de instrucciones, cero elisiones, y la retirada las apaga. Y **la «segunda forma de arista» no existe**: en la forma vigente una rama siempre termina la traza, así que un lazo se cierra siempre por un enlace y `JIT_FIN_LAZO` sólo aparece con `DCEMU_JIT_FLUJO=1` — el censo de fronteras de CT no lo lista. **El parque KOS elide y sale idéntico**: barrido de 151 demos con las 151 dejando captura, piso de ruido 0, señal 0 y cero veredictos serial distintos, y de su propio `stderr` sale el censo del parque — 46 831 297 sondas, 23 351 381 elisiones, `despacho` otra vez como única causa de fallo (23 466 708 contra 98 800 / 77 376 / 100 666) y **siete** vueltas sin lugar antes del corte en todo el parque. **Seis demos concentran todo y eliden el 63-65 % de sus instrucciones** (`cdrom-stream`, las dos de `filesystem-sd`, `library` y las dos de `pvr-yuv_converter`): las que se quedan esperando hardware que no está. **La red de juegos, once imagenes, once exactas** (intérprete contra traductor sobre un binario, captura byte a byte y puntos exactos, con el RTC clavado). El residuo nombrado y sin cliente conocido son los enlaces **dinámicos** de retroceso (9385 en CT). Ver `docs/recompilador-plan.md`, la sección final |
...
| `DCEMU_AICA_DEMORA_LINEA=N` | la **latencia fija de la línea del AICA al ASIC**, en muestras (2026-09-05; omisión **1** ≈ 22,7 µs emulados). Cada subida o bajada se anota con el sello de su muestra y `main_loop()` aplica en cada servicio, en orden causal, las selladas hasta `muestras(reloj_total) − N` — el mismo instante en los dos caminos, así que **`--hilos` es exacto hasta la lista de entregas en los tres guests, Sega Rally 2 incluido**, que era el único punto del hilo sin determinismo por construcción (llegaba 1052 ciclos tarde). Se aplica también sin hilos porque es una propiedad de la máquina emulada; `0` reproduce la línea base anterior byte a byte (el código viejo, textual: sus dos lazos «subidas y luego bajadas» no son el orden causal). Medida neutra sin hilos (DOOM +0,2 %, CT −1,3 %, SR2 −0,1 %, solapados). Contadores de control en el resumen: `perdidos` e `imposibles`, siempre cero. La lección pagada: `aica_muestras` no es señal de terminación (se incrementa antes de los temporizadores y del ARM); la señal es `aica_muestras_listas`, al final del tick. `herramientas/linea-gate.ps1` y `linea-ab.ps1`; expediente en `docs/hilos-plan.md` |
| `DCEMU_HILO_AICA_GIRO=N` | vueltas de `_mm_pause` que cada lado del hilo del AICA espera activo antes de dormirse (omisión 2000; sólo actúa con `--hilos`). Con la entrega determinista el hilo principal tiene que haber terminado la muestra del horizonte, y en Crazy Taxi llegaba tarde al **35 % de los bordes** — no por la holgura (D=2 y D=4 daban 28 y 26 %) sino porque el hilo se dormía tras cada muestra y el despertar cuesta más que los 9 µs de un intervalo a 2,5×. Con 2000 las esperas caen al 0,01 % en los tres guests. **Pero el hilo sigue apagado por omisión**: en la ventana del banco CT es neutro (−0,7 % solapado a 180 s; el −5,8 % del día anterior y el −9,5 % del giro eran de la ventana de 60 s) y DOOM +9,3 % y SR2 +6,5 % con rangos disjuntos en contra — el núcleo que gira le quita frecuencia al principal en este portátil. **La misma tarde el hilo pasó a dormir hasta el próximo borde de muestra** en vez de despertarse en cada publicación (diez de cada once eran para no mezclar nada, 500 000 avisos por segundo en CT), «al día» pasó a ser por cuenta de muestras (`aica_muestras_listas` contra `muestras(objetivo)`), compuerta verde otra vez, y las sondas se dieron vuelta: **CT −9,1 %, DOOM −7,9 %, SR2 −4,8 %** con giro. Pero la tanda de cuatro rondas sobre el canónico `A111161DB00F27C9` corrió con el portátil en uso y dio +12,8 / +5,9 / +15,6 % en contra, con los brazos sin hilos 3-4 % más lentos que su repetición anterior: **contaminada y por repetir en reposo**. **La repetición llegó la misma noche y no era el usuario** (la última entrada de la máquina fue la noche anterior): era Windows 11 estrangulando al proceso sin audio de una sesión desatendida (ver `DCEMU_ESTRANGULAR`), y un proceso en núcleos eficientes es el que peor lleva dos hilos calientes. Con el proceso exento, en reposo, sobre el canónico `E8D3B903B7B5C52A`: **DCDoom −8,3 %, Sega Rally 2 −7,1 %, Crazy Taxi −10,5 %, los tres con rangos disjuntos y 4/4** (`docs/hilos-plan.md`, sección final). **`--hilos` es la omisión desde el 2026-09-05** y `--sin-hilos` la palanca de aislamiento; los tres guiones cuyo brazo «sin» callaba la ponen explícita. `DCEMU_HILO_AICA_PRIORIDAD=1` (prioridad alta del hilo) medida neutra: no mueve las esperas |
| `DCEMU_SIN_RELOJ_EVENTOS=1` | apaga el reloj por eventos: el bloque periódico completo corre en cada frontera de 400 ciclos, **el comportamiento anterior bit a bit**. **Bajo `--hilos` corre también desde la noche del 2026-09-05** (antes el hilo lo apagaba: «ese camino publica trabajo en cada frontera»; la publicación necesita cada vencimiento, no cada grano, y el término del AICA sale del reloj — `aica_primer_reloj_de_muestra(aica_muestras_al_reloj() + 1)` — en vez de la cuenta atrasada del hilo). Compuerta de cinco brazos verde sobre el canónico reentrenado `FF19A0810D05593E`; servicios de CT **30 M → 5,3 M por minuto** y el bloque periódico de 12,6 % a 8,0 % en `--perf`… **que el reloj no vio**: SR2 −1,5 % con 4/4 y solape de 81 ms, CT −0,8 % con 3/4 solapado, DOOM inerte. El 12,6 % era **el instrumento**: 23 ns por marca sobre 90 millones de servicios son 2,1 s de los 7,9 (calibrados y descontados desde entonces en `--perf`, línea «de eso el instrumento»), y el resto es que un servicio medido corre serializado detrás de la marca mientras el servicio vacío de siempre se ejecuta en la sombra del trabajo vecino — la lección de B.3 con el instrumento de protagonista. Queda encendido como el atajo P1/P2: exacto, estrictamente menos trabajo y un caso especial menos; `docs/clock-plan.md`, sección final (barrido KOS 139/139 idéntico entre estados). Encendido, el servicio corre solo al llegar el próximo vencimiento (línea, muestra del AICA memoizada, TMU, WDT, demoras del INTC; con DMA auto no se saltea) o cuando alguien invalida con `reloj_tocar()` — la grilla, `reloj_total` y cada entrega no se mueven, por eso es exacto por construcción. Vale **−0,7/−1,0 % en SR2 y −0,5 % en CT, neutro en DOOM** (el guest denso en invalidaciones). Las tres reglas que lo sostienen (invalidar todo lo que mueva una entrega, sincronizar ticks antes de una escritura on-chip, el contador `reloj_toques` que impide que el recálculo pise una invalidación del propio servicio) están en `docs/clock-plan.md`, fase 5 |
...
- **`--sin-audio` costs nothing — but a covered window without audio did, and it read as
  ...
  which is exactly the regime of a detached bench on a machine Windows deems unattended (no
  keyboard or mouse input since the night before; a switched-off display or the lock screen
  covers every window) — and why the
...
- **A two-binary tanda needs a guest the change cannot touch: that guest is the noise gauge, and it has read 0.7 % — bigger than what was being measured.** Measured 2026-09-09, growing the MMU translation-cache entry from 28 to 32 bytes (a struct size, so no lever is possible): Crazy Taxi runs in flat mode, emits no translation at all and its code does not move, and it still came out **−0.7 % with disjoint ranges and 4/4 pairs** between the two PGO-retrained binaries — which is exactly the verdict the tree's standard calls real. With that floor, DCDoom's +0.4 % and Sega Rally 2's −0.1 % in the same table say nothing in either direction. Two consequences: run the inert guest even though it has nothing to contribute to the verdict, and **do not repeat to fix it** — the layout bias is systematic per binary, not random, so more rounds shrink the ranges and leave it intact. Nor can the inert guest's delta be subtracted from the others: its bias belongs to its own hot code. When the question splits into a half that fits a runtime lever and a half that does not, the lever's tanda is the one that decides.
- **When a guest's dispersion swallows the effect, change the statistic, not the conclusion: accumulated pairs across two independent tandas, calibrated by an inert guest, say more than four disjoint rounds.** Measured 2026-09-09 (the MMU tag merged with the permission): ranges overlapped in all six rows, so the strict standard did not apply — and DCDoom still won **10 of 10 pairs** across a 4-round and a 6-round tanda with the same −0.7 % both times, Sega Rally 2 9 of 10, while Crazy Taxi, **inert by construction** (flat mode emits no translation, so both arms are the same code), came out at **5 of 10 pairs and ±0.0 %** — exactly what no effect looks like. That control is what turns the pair count into evidence: 10/10 against a witness reading 5/10 is not the same claim as 4/4 with no witness. Decide the second tanda's recipe **before** looking at the first one's numbers and publish both tables, or this is just the post-hoc selection the rest of this section exists to prevent.
...
- **Un barrido en el que ninguna demo arrancó sale «piso 0, señal 0» y se lee como verde.** Pasó
  el 2026-09-04: faltaba `ip.bin` en el directorio de trabajo, las 151 demos salieron con `rv=1` y
  **cero bytes de captura**, los tres brazos coincidieron en nada y el resumen dijo lo que dice un
  barrido perfecto. Es la forma de falla recurrente del árbol —algo que se acepta sin hacer nada y
  sin decirlo— aplicada al instrumento en vez de al emulador. `barrido-jit.ps1` ahora imprime
  **cuántas demos dejaron captura** y falla si son cero, nombrando `ip.bin`; la regla general es
  que un resultado nulo perfecto se mira dos veces antes de creerlo, y que `resumen.csv` con
  `sha256` en `-` para todo es la firma.
- **Un juego lee el reloj a mitad de corrida, así que una compuerta que no clava el RTC compara dos
  relojes.** `DCEMU_RTC_FIJO` no es sólo para el barrido: `chd-exactitud.ps1` lo llevaba desde el
  expediente de THPS2 y `ociosos-gate.ps1` no, y salió **verde tres veces por suerte** antes de que
  Crazy Taxi divergiera **1852 instrucciones de 7478 millones en un solo brazo, con la captura byte
  a byte intacta y los otros tres brazos al dígito**. Esa es la firma: una divergencia diminuta, en
  un brazo cualquiera, con la imagen igual — que se lee como un bug del traductor y es el reloj.
  Con el RTC clavado en los cuatro brazos vuelve a verde y a los totales de siempre.
...
- **Vaciar una variable no es borrarla, y para `DCEMU_JIT` vaciarla apaga el traductor.**
  `getenv()` devuelve `""` --no NULL-- y `atoi("")` es 0, que es justo el valor de la palanca
  de aislamiento. En PowerShell 7 **`[Environment]::SetEnvironmentVariable($v, $null)` NO
  borra: deja la variable vacia** (`Test-Path Env:\X` sigue dando True), asi que un guion que
  "limpia" el ambiente asi corre el intérprete en todos sus brazos: la compuerta compara
  intérprete contra intérprete, sale **verde y muda**, y no prueba nada. La forma correcta es
  `Remove-Item "Env:NOMBRE" -ErrorAction SilentlyContinue`. Es la cara nueva de la trampa que
  la adopción ya cobró una vez (los siete guiones cuyo brazo de control borraba la variable).
  **El control barato que la caza**: que cada brazo imprima una línea que sólo existe si el
  traductor corrió — `herramientas/contrato-gate.ps1` falla si el brazo del intérprete la
  tiene, o si a un brazo del traductor le falta.
- **El resumen `jit:` no llegaba a `stderr.txt` salvo con `--perf`, y las compuertas corren
  sin `--perf`.** `jit_resumen()` estaba registrado con `atexit()` y corría después de que SDL
  cerrara la redirección, así que todos sus contadores --incluidos los de control-- eran
  invisibles exactamente en las corridas que los necesitan. Ahora lo llama `main.c` en la
  secuencia de salida, al lado de `arm7jit_resumen()`. Si un contador de control sale vacío,
  la primera pregunta es si la línea existe, no si el contador vale cero.
...
- **Los manejadores de `opcodes[]` tienen que conservar direcciones distintas.**
  ...
  `nop` con `NOIMP` y `shal91` con `shll94`, con lo que el traductor clasificaba palabras que no
  son instrucciones como NOP: exacto de casualidad, pero las trazas cambian de forma y los
  conteos dejan de ser invariantes entre compilaciones. **MSVC también pliega desde el toolset
  14.51 (VS 18)**: la guarda de `jit_iniciar()` saltó en el primer arranque del binario MSVC del
  árbol el 2026-09-03 («el enlazador plegó manejadores»), así que los dos enlaces van con
  `/OPT:NOICF` y la regla ya no es por compilador. La guarda sigue nombrándolo si vuelve, porque
  el síntoma — contadores del traductor distintos con captura exacta — no se parece en nada a la
  causa.
