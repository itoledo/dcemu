...
`UpdateSR()` entries — and the periodic block's own condition tests it, so the block runs
early whenever a request becomes deliverable.
...
**Two things about it were measured wrong first, and both are worth keeping.** The flag is
tested **inside the condition of the periodic block** (`>= RELOJ_GRANO || intc_sh4_reintentar`),
not in an `if` of its own: with its own branch Crazy Taxi paid **8.5%** (62 007 ms against
57 152 without the flag at all), folded into the branch that was already being evaluated it
pays nothing measurable (56 790). Bringing the whole block forward on a request is harmless —
it happens about a thousand times per emulated second and its consumers carry their own
remainders.

And **the flag, not the cadence, is what carries the correctness**: DCDoom gives the same 977
scenes and the same capture with `RELOJ_GRANO` at 400, 800 and 1600. That is what makes it a
fix rather than a lucky constant — it imitates what the chip does, delivering at the first
instruction boundary where the conditions hold, instead of hitting the right grain. Raising
the block past 400 buys 0.6% and is not worth the churn.

Both facts only came out after a clean re-measurement killed two false readings, and the two
traps cost three numbers in this section: a `dcemu.exe` **orphaned by a stopped run** was
eating a core through the first perf batch, and `git checkout -- <file>` restores from the
**index**, so a build that was supposed to carry the flag did not. Kill stray processes and
`git reset --hard` before any A/B here.
