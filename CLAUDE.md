...
| `DCEMU_JIT=1\|2` | el recompilador dinámico. **Sólo existe con `-DDCEMU_JIT=ON`**, como la fusión y por lo mismo: el A/B corre sobre una sola imagen. Con `1` corren los dos bloques que la fase 0 emitió a mano (Crazy Taxi **en paridad con la sonda de fusión**, DCDoom al 71 %); con `2`, el traductor automático de la fase 1 — 50 plantillas, **exacto al dígito y con las capturas byte a byte en los dos guests**, cubriendo el 79,9 % de DCDoom y el 72,4 % de Crazy Taxi con 18,2/9,2 instrucciones por entrada (los cuatro lotes de plantillas del censo: el push/pop de PR ya no corta los prólogos, los caminos de 16 bits existen en ambos sentidos, y la división y el MAC.L corren por el manejador real — la plantilla que llama al intérprete, con su lista blanca por la regla de la falta; `macl62` se reordenó para cumplirla, verificado con SingleStepTests). **Medido con PGO en los dos binarios** (el del JIT lleva su propio `dcemu-jit.pgd`; ver arriba): DCDoom **a 1,062× tiempo real — supera la consola** (32 957 ms el banco de 35 s); Crazy Taxi ~99,3 s, con su regla de lectura escrita en el plan: sus absolutos se comparan dentro de un binario o no se comparan (la capa de reentrenamiento le vale ±1-2 %). Llevar el JIT compilado con los ganchos apagados cuesta al intérprete **+3,6 %/+4,4 %** — la paridad entre binarios que antes no se podía separar del ruido. El redespacho en el arena por ayudante C se probó en dos formas y **pierde en las dos** (las llamadas fallidas superan a los aciertos; el viaje al despachador ya no es el costo dominante): está medido y revertido, ver el plan. Los bloques se encadenan con un `jmp` directo parcheado —los saltos indirectos incluidos, con una guarda de destino visto que el sitio aprende corriendo— y lo que lo hace sano son dos reglas: la **época** (una global que se mueve con un cambio de mapeo o con una escritura sobre una página con código traducido, de modo que la verificación por entrada son dos comparaciones y no el bucle de palabras) y el **puente**: directo sólo dentro de la misma ventana de 1 KB —misma página bajo cualquier tamaño, para siempre—, y entre páginas un talón por salida que hace la búsqueda real de instrucción (avance de `URC` y falta de TLB incluidos, hasta el conteo de fallos sale byte-idéntico) antes de saltar. `DCEMU_JIT_SIN_PUENTES=1` lo aísla. Compilando con `-DDCEMU_FUSION=ON -DDCEMU_JIT=ON` el mismo binario da la comparación de tres |
