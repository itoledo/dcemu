...
the same bench but each guest runs **twice, once per form** (interpreter and translator,
...
| `DCEMU_JIT=1\|2` | el recompilador dinámico. **Sólo existe con `-DDCEMU_JIT=ON`**, como la fusión y por lo mismo: el A/B corre sobre una sola imagen. Con `1` corren los dos bloques que la fase 0 emitió a mano (Crazy Taxi **en paridad con la sonda de fusión**, DCDoom al 71 %); con `2`, el traductor automático de la fase 1 — 117 plantillas, **exacto al dígito y con las capturas byte a byte en los tres guests verificados** (DCDoom, Crazy Taxi y Sega Rally 2), cubriendo el **84,4 %** de DCDoom (cero emisiones fallidas), el **89,4 %** de Crazy Taxi y el 87,6 % de SR2 — la frontera de plantillas cerrada: lo que corta son palabras de datos — (los seis lotes del censo más **la FPU de `sz0` sin MMU**: los `FMOV.S` emitidos por el puntero de banco vivo, la aritmética por el manejador real con Enables=0 garantizado por `b->fpu`, y `macl62` reordenado para la regla de la falta, verificado con SingleStepTests). **La compuerta MMU+FPU está levantada** (2026-08-09): los 633 M de divergencia del expediente eran el **0x800 del cambio perezoso de contexto FPU de WinCE** — `run()` lo alza en el despacho antes de tocar nada y una fila FPU emitida lo salteaba, con un avance de URC de más y el banco del dueño anterior escrito —; el arreglo es SR.FD como bit 3 de la clave de modo (recalculado en `excepcion_actualizar_vigilancia`) más el corte del descubrimiento con FD puesto, verificado canónico en seis escenarios × dos guests. `DCEMU_JIT_SIN_FPU_MMU=1` la cierra (aislamiento y línea base anterior); `DCEMU_JIT_CORTE_EPOCA=1` enciende el corte de época tras escrituras en bloques MMU (protector del agujero teórico del orden de búsqueda, que ningún banco observa y cuesta 6-8 % de cobertura — el análisis en el plan); `-DDCEMU_SONDA_URC` compila la sonda de conservación de avances (`uc`/`ue`/`uv` en los puntos de control) que cerró la caza. Sega Rally 2, el cliente de la fase 2, corre **0,92× (−8,1 % contra su intérprete)** en su mejor tanda — su dispersión entre tandas es la mayor del parque (±3-6 %) porque **no está en el banco de entrenamiento de PGO**, la mejora señalada en el plan. Crazy Taxi: 92,3 s su mejor absoluto, **−16,3 % su mejor cociente** (superbloques paso 1: la ventana de 1 KB del descubrimiento quedó solo bajo MMU — donde vive el contrato de búsqueda — y `JIT_MAX_SALIDAS` se desfosilizó: estaba en el viejo tope y un bloque con más cortes desbordaba la emisión entera y quedaba interpretado para siempre; el tope de instrucciones a 96 se midió y perdió — el largo va a colas frías y el código disperso pesa más, ver el plan). La fase de registros persistentes está en su v1: colocación canónica para r0/r2/r3/r4/r15 con selección codiciosa intacta, y costuras vacías en los enlaces (B⊆A salta directo al cuerpo); el talón con cargas parciales perdió su A/B y quedó conmutable. **Medido con PGO en los dos binarios** (el del JIT lleva su propio `dcemu-jit.pgd`; ver arriba): DCDoom **a 1,078× tiempo real — supera la consola** (32 469 ms el banco de 35 s); Crazy Taxi **92,3 s (−15,5 % contra su intérprete, dispersión 0,03 %)** — la aritmética FPU va por el manejador real con el envoltorio ligero, las seis filas sin FPSCR emitidas enteras, y los pares de sz1 en un acceso de 8 bytes, con su regla de lectura escrita en el plan: sus absolutos se comparan dentro de un binario o no se comparan. Llevar el JIT compilado con los ganchos apagados cuesta al intérprete **+3,6 %/+4,4 %** — la paridad entre binarios que antes no se podía separar del ruido. El redespacho en el arena por ayudante C se probó en dos formas y **pierde en las dos** (las llamadas fallidas superan a los aciertos; el viaje al despachador ya no es el costo dominante): está medido y revertido, ver el plan. Los bloques se encadenan con un `jmp` directo parcheado —los saltos indirectos incluidos, con una guarda de destino visto que el sitio aprende corriendo— y lo que lo hace sano son dos reglas: la **época** (una global que se mueve con un cambio de mapeo o con una escritura sobre una página con código traducido, de modo que la verificación por entrada son dos comparaciones y no el bucle de palabras) y el **puente**: directo sólo dentro de la misma ventana de 1 KB —misma página bajo cualquier tamaño, para siempre—, y entre páginas un talón por salida que hace la búsqueda real de instrucción (avance de `URC` y falta de TLB incluidos, hasta el conteo de fallos sale byte-idéntico) antes de saltar. `DCEMU_JIT_SIN_PUENTES=1` lo aísla. Compilando con `-DDCEMU_FUSION=ON -DDCEMU_JIT=ON` el mismo binario da la comparación de tres |
...
| `DCEMU_JIT_SONDA_CRUCES=1` | un contador emitido en la cabeza de cada bloque: cruces de enlace (corridos − entradas; 38-47 % de las fronteras) y el censo de presencia de registros ponderado por veces, ambos al salir |
