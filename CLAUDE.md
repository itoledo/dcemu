...
| `DCEMU_JIT=1\|2` | el recompilador dinámico. **Sólo existe con `-DDCEMU_JIT=ON`**, como la fusión y por lo mismo: el A/B corre sobre una sola imagen (build `build-jit/`, con su propio perfil `dcemu-jit.pgd` — ver arriba). Con `1` corren los dos bloques emitidos a mano de la fase 0; con `2`, el traductor automático: **121 plantillas elegidas por censo, exacto al dígito y con las capturas byte a byte en los tres guests verificados** — DCDoom (MMU, 89,8 % de cobertura y **43,2 instrucciones por entrada**, **1,10× tiempo real, −25,5 %** contra su intérprete), Crazy Taxi (90,1 %; 92,3-92,5 s su mejor absoluto, −16,7 % su mejor cociente) y Sega Rally 2 (MMU+FPU, 92,9 %, 0,91×, −9,6 %) —, con la frontera de plantillas gobernada por **el censo de la frontera por peso** (en el resumen `jit:`: en qué termina cada bloque, ponderado por veces — lo que separa «hay sitios» de «por ahí pasa la ejecución»): lo que corta son palabras de datos, escritores de SR, `TRAPA` y las ranuras con memoria de las ramas sin par. **Los pares de rama emitidos** (`rts`/`bra`/`jmp`/`braf` + ranura con memoria, el epílogo Katana y parientes) son lo que bajó las entradas 50 % en DOOM y 23 % en SR2; JSR/BSR/BSRF quedan sin par (escriben PR antes de la ranura y una falta lo revertiría por instantánea en el intérprete, no en el emitido), y **los ciclos de la rama van después de la ranura** — sumarlos antes se evapora en la recarga de CYC de una ranura por manejador, ±15-19 k instrucciones de divergencia con la captura intacta. Lo que lo hace sano son la **época** con SR.MD en la clave de validez, el **puente** entre páginas (`DCEMU_JIT_SIN_PUENTES=1` lo aísla), y la **clave FPU con SR.FD como bit 3** — la compuerta MMU+FPU está levantada: su divergencia de 633 M era el 0x800 del cambio perezoso de contexto FPU de WinCE, que `run()` alza antes de tocar nada y un bloque salteaba. La regla de lectura: **los absolutos se comparan dentro de un binario o no se comparan** (la capa de reentrenamiento vale ±1-2 % en CT; la de SR2 era ±3-6 % hasta que entró al banco de entrenamiento, 2026-08-09). Llevar el JIT compilado con los ganchos apagados cuesta al intérprete +3,6 %/+4,4 %. **El estado, las reglas completas, los veredictos medidos (redespacho, buscador, talones de costura, tope de 96 — todos probados y descartados con su porqué) y lo pendiente viven en `docs/recompilador-plan.md`**, ahora en forma de conclusiones; la bitácora entera está en git. Compilando con `-DDCEMU_FUSION=ON -DDCEMU_JIT=ON` el mismo binario da la comparación de tres |
...
| `DCEMU_JIT_SIN_PARES=1` | apaga los pares de rama (`rts`/`bra`/`jmp`/`braf` + ranura con memoria) y reproduce la frontera anterior al dígito. Vienen **encendidos**: con las 4 plantillas del censo valen el mayor salto de la serie (DOOM −25,5 % y 1,10×, entradas −50 %; SR2 −9,6 %; CT −16,7 %); el par **termina la traza** — dejar la cola le costó a SR2 dos puntos y +27 % de arena |
...
en git), `estado-del-arte-plan.md` (el plan maestro de rendimiento vigente: el orden de
las fases —superbloques, ARM7, reloj por eventos, fastmem— fijado por el reparto medido
bajo el JIT, que `herramientas/perfil-jit.ps1` rehace tras cada fase),
